...
ResultScreen_t* CreateResultScreen(void) {
    ...
    // Positive effects box (center-right, next to "YOU WIN" message) - runtime positioning
    screen->positive_effects_box = a_FlexBoxCreate(
        GetWindowWidth() / 2 + 120,  // X: Just to the right of center message
        GetWindowHeight() / 2 - 60,  // Y: Aligned with "YOU WIN" message
    ...
    // Negative effects box (center-right, stacked below positive effects) - runtime positioning
    screen->negative_effects_box = a_FlexBoxCreate(
        GetWindowWidth() / 2 + 120,  // X: Same alignment as positive box
        GetWindowHeight() / 2 + 20,  // Y: Just below center, stacks under positive
...
void ShowResultScreen(ResultScreen_t* screen, int old_chips, int chip_delta, int status_drain, bool is_victory, int cleared_effects_count) {
    ...
    // ========================================================================
    // Populate FlexBox effects (auto-positioned, no collision detection!)
    // ========================================================================
...
void UpdateResultScreen(ResultScreen_t* screen, float dt, TweenManager_t* tween_mgr) {
    ...
    // Tween chip counter (slot machine effect) after delay
    if (screen->timer > 0.3f && screen->display_chips != (float)screen->old_chips + screen->chip_delta - screen->status_drain) {
        int target_chips = screen->old_chips + screen->chip_delta - screen->status_drain;
        TweenFloat(tween_mgr, &screen->display_chips, (float)target_chips, 0.8f, TWEEN_EASE_OUT_CUBIC);
    }
}

void RenderResultScreen(ResultScreen_t* screen, Player_t* player, GameState_t state) {
    if (!screen || !player) return;
    (void)state;  // Unused - player->state determines message

    // Semi-transparent overlay (runtime window size)
    a_DrawFilledRect((aRectf_t){0, 0, GetWindowWidth(), GetWindowHeight()}, (aColor_t){0, 0, 0, OVERLAY_ALPHA});
    ...
    // Draw result message (runtime centered)
    ...
    a_DrawText((char*)message, GetWindowWidth() / 2, GetWindowHeight() / 2 - 40, result_config);
    ...
    // Measure chip text width for positioning damage text (runtime centered)
    float chip_text_width = 0;
    float chip_text_height = 0;
    a_CalcTextDimensions((char*)d_StringPeek(chip_info), FONT_STYLE_TITLE.type, &chip_text_width, &chip_text_height);
    int chip_center_x = GetWindowWidth() / 2;
    int chip_y = GetWindowHeight() / 2 + 20;
    ...
    // ========================================================================
    // Render FlexBox effects (auto-positioned, no collision detection!)
    // ========================================================================
